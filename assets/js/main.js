...
// ===== KNOWLEDGE SLIDER =====
const knowSlides = document.querySelectorAll(".knowledge-slider img");
let knowIndex = 0;

setInterval(()=>{
knowSlides[knowIndex].classList.remove("active");
knowIndex = (knowIndex + 1) % knowSlides.length;
knowSlides[knowIndex].classList.add("active");
},5000);


// ===== SCHEDULE SLIDER =====
const scheduleSlides = document.querySelectorAll(".slider-track img");
...
scheduleSlides.forEach(img=>img.classList.remove("active"));
scheduleSlides[index].classList.add("active");
}

// next
nextBtn.addEventListener("click", ()=>{
scheduleIndex = (scheduleIndex + 1) % scheduleSlides.length;
showSchedule(scheduleIndex);
});

// prev
prevBtn.addEventListener("click", ()=>{
scheduleIndex = (scheduleIndex - 1 + scheduleSlides.length) % scheduleSlides.length;
showSchedule(scheduleIndex);
});
